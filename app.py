...
#                 result = app.invoke(
#                     {"messages": state_messages := [*st.session_state.get("langgraph_messages", []), current_turn_message]},
...
def call_model(state: MessagesState):
    system_prompt = (
        ...
        "You MUST use both the conversation history and the retrieved document context.\n"
        "If the user asks about previous questions, answer using chat history.\n"
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            try:
                ...
                df = pd.DataFrame(
                    [(doc[0].page_content, doc[0].metadata.get("source"), doc[0].metadata.get("page"), doc[1]) for doc in docs],
                    columns=["content", "source", "page", "score"]
                )
                ...
                source = df["source"].iloc[0] if not df.empty else "N/A"
                pages = ", ".join(df["page"].dropna().astype(str).unique()[:3])

                final_response = (
                    f"{answer}\n\n"
                    f"**Source Document:** {source}\n"
                    f"**Reference Pages:** {pages}"
                )
